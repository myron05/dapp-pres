...
export default function Info() {
  ...
  let lastRemainToken = ethers.utils.formatUnits('37', 18);
  let lastTokenSold = ethers.utils.formatUnits('10', 18);
      ...
      <Row justify="space-around">
        ...
        <Col md={{ span: 10, offset: 1 }} lg={{ span: 7, offset: 0 }}>
          <CardStyled>
            ...
            <Text>{new Intl.NumberFormat().format(Number(lastRemainToken) + Number(remainToken))} DIGL</Text>
          </CardStyled>
        </Col>
        <Col md={12} lg={7}>
          <CardStyled>
            <SubTitle>Token Sold</SubTitle>
            <Text>{new Intl.NumberFormat().format(Number(tokenSold) + Number(lastTokenSold))} DIGL</Text>
          </CardStyled>
        </Col>
      </Row>
    </Container>
